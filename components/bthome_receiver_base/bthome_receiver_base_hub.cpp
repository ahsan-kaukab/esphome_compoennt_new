...
namespace esphome
{
  namespace bthome_receiver_base
  {
    ...
    uint64_t load_mac_address(uint32_t key) {
        ...
        // ESPPreferenceObject pref_obj = preff.make_preference(8,key);
        ...
        // Convert 6-byte MAC address to uint64_t
        uint64_t mac = 0;
        for (int i = 0; i < 6; i++) {
    ...
    BTHomeReceiverBaseDevice *BTHomeReceiverBaseHub::add_device(uint32_t key_address)
    {
      mac_address_t address  = load_mac_address(key_address);
